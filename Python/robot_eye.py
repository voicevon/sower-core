...
class myCamera(object):
    ...
    def config(self, config_file=None, TriggerMode=2, TriggerType=0, AeState=False, ExposureTime=30):
        # 获取相机特性描述
    ...
    def corn_recognition(self, raw_img, ROI=None, display=False, theshold_R=200, theshold_G=200, theshold_B=150,
        ...
        if self.ROI is not None:
            ...
            img_mask = self.corn_img[(self.corn_img[:, :, 0] < theshold_R) | (self.corn_img[:, :, 1] < theshold_G) |
                                     (self.corn_img[:, :,
                                      2] > theshold_B)] = 0  # make mask | (self.corn_img[:, :, 2] < 100)
...
class RobotEye(object):
    ...
    def __main_task(self):
        ...
        if self.__camera.isopen:
            while True:
                if self.__message_id != message_id:  # camera initial
                    if self.__camera_config.get('config file', "") != "":
                        self.__camera.config(self.__camera_config['config file'])
                    else:
                        trigger_mode = self.__camera_config.get(['trigger mode'], 2)
                        trigger_type = self.__camera_config.get(['trigger type'], 0)
                        aestate = self.__camera_config.get(['aestate'], False)
                        exposure_time = self.__camera_config.get(['exposure time'], 10)
                        self.__camera.config(TriggerMode=trigger_mode, TriggerType=trigger_type, AeState=aestate,
                                             ExposureTime=exposure_time)
                else:  # waiting on camera initial
                    continue
                if self.__camera.frame_id != last_frame_id:
                    if self.__detect_config.get('ROI', []):
                        if len(self.__detect_config['ROI']) == 4:
