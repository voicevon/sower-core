...
class corn_detection(object):
    ...
    def preprocessing(self, img, ROI=None, threshold_R=100, threshold_G=100, threshold_B=100,
    					threshold_H_L=15,theshold_H_H=50, theshold_S=40, theshold_V=45):
        if img.shape[2]!=3:
        ...
        self.HSV_img = cv2.cvtColor(self.corn_img,cv2.COLOR_BGR2HSV) # convert to HSV space
        self.BGR_img = self.corn_img.copy()

        #threshold HSV
        yellow_min = np.array([threshold_H_L, theshold_S, theshold_V], np.uint8)
        yellow_max = np.array([threshold_H_H, 255, 255], np.uint8)
        self.HSV_th_img = cv2.inRange(self.HSV_img, yellow_min, yellow_max)

        #threshold BGR
        yellow_min = np.array([0, threshold_G, threshold_R], np.uint8)
        yellow_max = np.array([threshold_B, 255, 255], np.uint8)
        self.BGR_th_img = cv2.inRange(self.BGR_img, yellow_min, yellow_max)
        #cv2.imwrite('test1.png', self.BGR_th_img)

        #median blur
        self.HSV_th_img = cv2.medianBlur(self.HSV_th_img,7)
        self.BGR_th_img = cv2.medianBlur(self.BGR_th_img,3)
        ...
        self.BGR_th_img = cv2.morphologyEx(self.BGR_th_img, cv2.MORPH_CLOSE, element)
	...
	def detect(self, display=False, theshold_size_rgb=15, theshold_size_hsv=25):
        cell_width = int (self.ROI_img.shape[1]/self.tray_width)
        cell_height = int (self.ROI_img.shape[0]/self.tray_height)
        ...
        for row in range(self.tray_height):
            for col in range(self.tray_width):
                ...
                cell_BGR_img = self.BGR_th_img[row*cell_height:(row+1)*cell_height,col*cell_width:(col+1)*cell_width]
                #cv2.imwrite('test2.png', cell_HSV_img)


                contours_HSV,hierarchy_HSV = cv2.findContours(cell_HSV_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                contours_BGR,hierarchy_BGR = cv2.findContours(cell_BGR_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                ...
                if len(areas) > 0:
                    if np.max(areas) > theshold_size_rgb:
                    	...
                    	if display:
                    	    max_area_id = np.argmax(areas)
                    	    cont_max_BGR = contours_BGR[max_area_id]
                    	    rect = cv2.boundingRect(cont_max_BGR)
...
class RobotEye(object):
        ...
        if self.__camera.isopen:
            ...
            # while True:   #TODO
                if self.__camera.frame_id != self.__last_frame_id:
                    ...
                    thres_H_L = self.__detect_config.get('threshold_H_L', 150)
                    ...
                    result = __corn_detect.detect(display, thres_size_rgb, thres_size_hsv)
                    # *************************************************************************
    ...
    def on_mqtt_message(self, topic, payload):
        ...
        if self.__tray_config.get('width') is not None and self.__tray_config.get('height') is not None and size_changed:
